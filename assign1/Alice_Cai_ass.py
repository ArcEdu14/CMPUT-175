    ...
    for i in range(len(data)):
        if i != 0:
            try:
                ...
                quit()
            if data[i] == "" or data[i][0] == "" or data[i][1] == "" or len(data[i]) != 4:
                print("country.txt is not formatted properly!")
                print("Exiting the program...")
                quit()
...
def check_product(data):
    for i in range(len(data)):
        if i != 0:
            if data[i] == "" or data[i][0] == "" or data[i][1] == "" or data[i][2] == "" or len(data[i]) != 3:
    ...
    for i in range(len(data)):
        if i != 0:
            try:
                ...
                print("Exiting the program...")
                quit()
            if data[i] == "" or data[i][0] == "" or data[i][1] == "" or len(data[i]) != 3:
                print("tariff.txt is not formatted properly!")
                print("Exiting the program...")
                quit()
...
def find_trade_deficits(country_list):
    ...
    for i in range(len(country_list)):
        country_list[i].append(country_list[i][2] - country_list[i][3])  # imports - exports
...
def products_per_industry(product_list):
    ...
    # because dictionaries are a pain to sort (I switched to dictionaries later)
    industry_list = []
    industry_number = []
    ...
    for i in range(len(industry_list)):
        industry_list[i] = [industry_list[i]]
        industry_list[i].append(industry_number[i])
    industry_list.sort()
    # convert back into two lists
    for i in range(len(industry_list)):
        industry_number[i] = industry_list[i][1]
        industry_list[i] = industry_list[i][0]
...
def identify_select_industries(tariff_masterlist, country_list):
    ...
    for i in range(len(tariff_masterlist)):
        for industry in ['Agriculture', 'Manufacturing', 'Tech', 'Pharmacy', 'Food']:  # all possible industries
...
def get_tariff_rate(country, industry, tariff_masterlist):
    ...
    for i in range(len(tariff_masterlist)):
        if country == tariff_masterlist[i][0]:
            ...
            return rate/100  # the rate as a fraction
...
def get_summary_data(table_list):
    ...
    :return: list
    """
    summary_data = [0, 0, 0]  # [cost before tariff, total tariff paid, grand total]
    for i in range(len(table_list)):
